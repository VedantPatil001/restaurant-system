...
def home():
    ...
    if search_query or category_filter:
        ...
        if search_query:
            conditions.append("(m.mname ILIKE %s OR fc.category ILIKE %s)")
            params.extend([f'%{search_query}%', f'%{search_query}%'])
        
        if category_filter:
            conditions.append("fc.category = %s")
            params.append(category_filter)
        
        query += " AND ".join(conditions)
    
    query += " ORDER BY m.mid"
    
    cur.execute(query, params)
    menus = cur.fetchall()
    
    # Get all categories for filter dropdown
    cur.execute("SELECT category FROM food_cat ORDER BY category")
    categories = cur.fetchall()
    
    cur.close()
    conn.close()

    return render_template("public_menu.html", menus=menus, categories=categories, 
                          search_query=search_query, category_filter=category_filter)

# ================= ADMIN MENU SEARCH =================
@app.route('/admin/menu/search')
def admin_menu_search():
    ...
    return render_template("admin_menu_search.html", menus=menus, 
...
def admin_orders_search():
    ...
    if search_query:
        query += " AND (u.username ILIKE %s OR o.oid::text ILIKE %s OR COALESCE(o.table_number, '') ILIKE %s)"
    ...
    return render_template("admin_orders_search.html", orders=orders, 
...
def admin_users_search():
    ...
    query = """
        SELECT id, username, email, role, 
               TO_CHAR(created_at, 'DD/MM/YYYY HH12:MI AM') as joined_date
        FROM users 
        WHERE 1=1
    """
    ...
    return render_template("admin_users_search.html", users=users, 
                          roles=roles, search_query=search_query,
                          role_filter=role_filter)

# [All your existing routes remain exactly the same from here...]
# [I'll keep them unchanged to maintain all functionality]
...
def cancel_order(oid):
    ...
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute("SELECT status FROM orders WHERE oid=%s AND user_id=%s", (oid, session['user_id']))
    order = cur.fetchone()
    if order and order[0] == 'Pending':
        cur.execute("UPDATE orders SET status='Cancelled' WHERE oid=%s", (oid,))
        conn.commit()
...
def order_details(oid):
    ...
    cur.execute("""
        SELECT 
            o.oid, o.total_price, o.status, o.payment_method, o.payment_status, o.table_number,
            TO_CHAR(o.created_at AT TIME ZONE 'UTC' AT TIME ZONE 'Asia/Kolkata', 'DD/MM/YYYY HH12:MI AM') as order_date,
            TO_CHAR(o.updated_at AT TIME ZONE 'UTC' AT TIME ZONE 'Asia/Kolkata', 'DD/MM/YYYY HH12:MI AM') as last_updated
        FROM orders o
        WHERE o.oid=%s
    """, (oid,))
    order = cur.fetchone()
    cur.execute("SELECT m.mname, oi.qty, oi.price FROM order_items oi JOIN menu m ON m.mid = oi.menu_id WHERE oi.order_id=%s", (oid,))
    items = cur.fetchall()
    total = 0
    for item in items:
        total += item[1] * item[2]
...
def admin_dashboard():
    ...
    cur.close()
    conn.close()
    return render_template("admin_dashboard.html", total_orders=total_orders, total_sales=total_sales)
...
def invoice(oid):
    ...
    for item in items:
        ...
        total += item[2]*item[1]
    content += f"\nTotal: ₹ {total}"
    return Response(content, mimetype="text/plain", headers={"Content-Disposition": f"attachment;filename=invoice_{oid}.txt"})
...
def dashboard():
    ...
    cur.execute("SELECT id, username, email, role, created_at FROM users ORDER BY id DESC LIMIT 5")
    recent_users = cur.fetchall()
    cur.close()
    conn.close()
    return render_template("dashboard.html", total_orders=total_orders, total_revenue=total_revenue,
                          pending_orders=pending_orders, pending_payment_orders=pending_payment_orders,
                          total_menu=total_menu, total_categories=total_categories, total_sizes=total_sizes,
                          total_users=total_users, total_viewers=total_viewers, total_admins=total_admins,
...
def edit_category(id):
    ...
    if request.method == 'POST':
        category = request.form['category']
        cur.execute("UPDATE food_cat SET category=%s WHERE fid=%s", (category, id))
        conn.commit()
...
def edit_quantity(id):
    ...
    if request.method == 'POST':
        size = request.form['size']
        cur.execute("UPDATE qty_mast SET size=%s WHERE qid=%s", (size, id))
        conn.commit()
...
def menu_list():
    ...
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute("SELECT m.mid, m.mname, m.price, fc.category, qm.size FROM menu m JOIN food_cat fc ON fc.fid = m.fid JOIN qty_mast qm ON qm.qid = m.qid ORDER BY m.mid")
    menus = cur.fetchall()
    cur.close()
    conn.close()
...
def add_menu():
    ...
    if request.method == 'POST':
        ...
        cur.execute("INSERT INTO menu (mname, price, fid, qid) VALUES (%s, %s, %s, %s)", (mname, price, fid, qid))
        conn.commit()
    ...
    cur.close()
    conn.close()
    return render_template("add_menu.html", categories=categories, quantities=quantities)
...
def edit_menu(id):
    ...
    if request.method == 'POST':
        ...
        fid = request.form['fid']
        qid = request.form['qid']
        cur.execute("UPDATE menu SET mname=%s, price=%s, fid=%s, qid=%s WHERE mid=%s", (mname, price, fid, qid, id))
        conn.commit()
    ...
    cur.close()
    conn.close()
    return render_template("edit_menu.html", menu=menu, categories=categories, quantities=quantities)
...
def view_cart():
    ...
    for mid, qty in cart.items():
        ...
        if menu:
            subtotal = menu[2] * qty
            total += subtotal
            items.append({'mid': menu[0], 'name': menu[1], 'price': menu[2], 'qty': qty, 'subtotal': subtotal})
    cur.close()
    conn.close()
...
def admin_orders():
    ...
    cur = conn.cursor()
    cur.execute("""
        SELECT o.oid, u.username, o.total_price, o.status, o.payment_method, 
               o.payment_status, o.payment_id, o.table_number,
               TO_CHAR(o.created_at AT TIME ZONE 'UTC' AT TIME ZONE 'Asia/Kolkata', 'DD/MM/YYYY HH12:MI AM') as order_date,
               TO_CHAR(o.updated_at AT TIME ZONE 'UTC' AT TIME ZONE 'Asia/Kolkata', 'DD/MM/YYYY HH12:MI AM') as last_updated
        FROM orders o
        JOIN users u ON u.id = o.user_id
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            for mid, qty in cart.items():
                ...
                cur.execute("INSERT INTO order_items (order_id, menu_id, qty, price) VALUES (%s, %s, %s, %s)", (order_id, mid, int(qty), price))
            conn.commit()
            if payment_method == 'Cash':
                session.pop('cart', None)
                flash(f"Order placed successfully! Please pay ₹{total} on delivery.", "success")
                return redirect('/my_orders')
            elif payment_method == 'Online':
                razorpay_order = razorpay_client.order.create({'amount': int(total * 100), 'currency': 'INR', 'payment_capture': '1'})
                cur.execute("UPDATE orders SET razorpay_order_id=%s WHERE oid=%s", (razorpay_order['id'], order_id))
                conn.commit()
                session['pending_order_id'] = order_id
                return render_template('online_payment.html', order_id=order_id, total=total,
                                     razorpay_order_id=razorpay_order['id'], razorpay_key_id=RAZORPAY_KEY_ID)
        except Exception as e:
            conn.rollback()
            flash(f"Something went wrong while placing order: {e}", "error")
            print("Error:", e)
            return redirect('/cart')
        finally:
            cur.close()
...
def payment_success():
    ...
    generated_signature = hmac.new(key=RAZORPAY_KEY_SECRET.encode('utf-8'), msg=f"{razorpay_order_id}|{payment_id}".encode('utf-8'), digestmod=hashlib.sha256).hexdigest()
    if generated_signature == signature:
        ...
        try:
            ...
            if payment_details.get('status') == 'failed':
                ...
                cur.execute("UPDATE orders SET payment_status='Failed', payment_id=%s, status='Payment Failed', payment_error=%s WHERE razorpay_order_id=%s", (payment_id, error_message, razorpay_order_id))
                conn.commit()
                flash(f"Payment failed: {error_message}", "error")
                return redirect('/my_orders')
            cur.execute("UPDATE orders SET payment_status='Completed', payment_id=%s, status='Pending' WHERE razorpay_order_id=%s", (payment_id, razorpay_order_id))
            if cur.rowcount > 0:
...
def payment_failed():
    ...
    if order_id:
        conn = get_db_connection()
        cur = conn.cursor()
        cur.execute("UPDATE orders SET status='Payment Failed' WHERE oid=%s AND status='Pending Payment'", (order_id,))
        conn.commit()
        cur.close()
        conn.close()
        session.pop('pending_order_id', None)
...
def retry_payment(order_id):
    ...
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute("SELECT total_price FROM orders WHERE oid=%s AND user_id=%s", (order_id, session['user_id']))
    order = cur.fetchone()
    if not order:
    ...
    try:
        razorpay_order = razorpay_client.order.create({'amount': int(total * 100), 'currency': 'INR', 'payment_capture': '1'})
        cur.execute("UPDATE orders SET razorpay_order_id=%s, status='Pending Payment' WHERE oid=%s", (razorpay_order['id'], order_id))
        conn.commit()
        session['pending_order_id'] = order_id
        return render_template('online_payment.html', order_id=order_id, total=total,
                             razorpay_order_id=razorpay_order['id'], razorpay_key_id=RAZORPAY_KEY_ID)
...
def admin_tables():
    ...
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute("SELECT id, username, email, role, table_number, created_at FROM users ORDER BY CASE WHEN table_number IS NULL THEN 1 ELSE 0 END, table_number, id DESC")
    users = cur.fetchall()
    cur.execute("SELECT table_number, COUNT(*) FROM users WHERE table_number IS NOT NULL GROUP BY table_number ORDER BY table_number")
    ...
    cur.close()
    conn.close()
    return render_template("admin_tables.html", users=users, occupied_tables=occupied_tables, available_tables=available_tables)
...
def assign_table(user_id):
    ...
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute("SELECT id FROM users WHERE table_number=%s AND id!=%s", (table_number, user_id))
    if cur.fetchone():
        flash(f"Table {table_number} is already assigned to another user!", "error")
        cur.close()
        conn.close()
        return redirect('/admin/tables')
    cur.execute("UPDATE users SET table_number=%s WHERE id=%s", (table_number, user_id))
    conn.commit()
    cur.close()
    conn.close()
...
def table_occupancy():
    ...
    cur = conn.cursor()
    cur.execute("""
        SELECT u.table_number, u.username, u.email, COUNT(o.oid) as order_count, MAX(o.created_at) as last_order
        FROM users u
